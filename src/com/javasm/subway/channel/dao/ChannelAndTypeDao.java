...
public class ChannelAndTypeDao implements  SelectChannelAndType {
	...
	public int SelectChannelAndTypeCount() {
		...
		ResultSet rs = null;
		conn=DruidHelp.getConnection();
		String sql = "select count(*) as count from pn_release_channel_type AS type1,pn_release_channel_type AS type2,pn_release_channel as channel WHERE channel.typeid = type1.tid AND type1.pid = type2.tid";
		try {
			ps =conn.prepareStatement(sql);
			rs=ps.executeQuery();
			while(rs.next()){
